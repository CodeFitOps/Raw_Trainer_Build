...
def _render_human_node(
    ...
    *,
    ...
    if isinstance(node, list):
        # A list itself doesn't know its key unless path encodes it, which we do.
        # We'll allow path rules at the list container, i.e. "...exercises[]"
        # plus user pattern "...exercises" also matches.
    ...
    if isinstance(node, dict):
        for k, v in node.items():
            ...
            if isinstance(v, list):
                ...
                # apply rule on this key (path+key or key-only) if it's a scalar-like decision (e.g., hide label)
                ...
                if r_key:
                    ...
                    if isinstance(render, dict) and render.get("show_label") is False and render.get("value") is not None:
                        # user asked "don't print the key, only the value" — for lists, we just print items with same indent
                        _render_human_node(
                            console,
                            theme,
                            schema=schema,
                ...
                # path-level special renderer (like exercises)
                ...
                # default: print label then list content (unless a key rule hides label)
                if r_key:
                    render = r_key.get("render") or {}
            ...
            # default scalar printing: KEY: value (key in role key, value plain)
    ...
    # scalar root
    _print_human_line(console, theme, indent, _style_scalar_value(node, theme, role="plain"), indent_spaces=indent_spaces)
